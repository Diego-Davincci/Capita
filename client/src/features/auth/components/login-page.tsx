...
/**
 * LoginPage — Google OAuth login screen.
 * Reads the `err` search param (set by the backend on OAuth failure)
 * and shows a toast notification if present.
 *
 * Test cases:
 * 1. Renders without errors when no `err` search param is present
 * 2. Shows a persistent toast notification when `err` param is set
 * 3. Google OAuth link points to the correct `API_URL/auth/google` endpoint
 * 4. "Continuar con Google" button is visible and clickable
 * 5. Redirects back here on failed OAuth (err param preserved in URL)
 */
